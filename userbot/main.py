...
from . import BRAIN_CHECKER, LOGS, bot, PLUGIN_CHANNEL_ID, CMD_HELP, LANGUAGE, OLRIC_VERSION, PATTERNS
...
def extractCommands(file):
    ...
    if re.search(r'CmdHelp\(.*\)', FileRead):
        ...
        for Command in Pattern:
            ...
            # OlricPY
            Olricpy = re.search('\"\"\"OLRICPY(.*)\"\"\"', FileRead, re.DOTALL)
            if not Olricpy == None:
                Olricpy = Olricpy.group(0)
                for Satir in Olricpy.splitlines():
...
try:
    bot.start()
    idim = bot.get_me().id
    olricbl = requests.get('https://gitlab.com/r3hm4nl1/olri/-/raw/master/olri.json').json()
    if idim in olricbl:
        bot.disconnect()
...
LOGS.info("Botunuz çalışıyor! Herhangi bir sohbete .alive yazarak Test edin."
          " Yardıma ihtiyacınız varsa, Destek grubumuza gelin t.me/OlricSupport")
LOGS.info(f"Bot sürümünüz: Olric {OLRIC_VERSION}")
