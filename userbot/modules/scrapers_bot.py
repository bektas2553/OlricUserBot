...
async def memeyap(event):
    ...
    if event.is_reply:
        ...
        if reply.photo:
            ...
            if os.path.exists("./OlricSticker.png"):
                os.remove("./OlricSticker.png")

            foto = await reply.download_media()
            im = Image.open(foto).convert("RGB")
            im.save("OlricSticker.png", "png")
            Resim = "OlricSticker.png"
        elif reply.sticker and reply.file.ext == ".tgs":
            sticker = await reply.download_media()
            os.system(f"lottie_convert.py --frame 0 -if lottie -of png '{sticker}' OlricSticker.png")
            os.remove(sticker)
            Resim = "OlricSticker.png"
        elif reply.media:
            Resim = await reply.download_media()
            Sure = os.system("ffmpeg -i '"+Resim+"' 2>&1 | grep Duration | awk '{print $2}' | tr -d , | awk -F ':' '{print ($3+$2*60+$1*3600)/2}'``")
            os.system(f"ffmpeg -i '{Resim}' -vcodec mjpeg -vframes 1 -an -f rawvideo -ss {Sure} OlricThumb.jpg")
            os.remove(Resim)
            Resim = 'OlricThumb.jpg'
        else:
            return await event.edit(LANG['REPLY_TO_MEME'])
            
        if os.path.exists("./olricmeme.png"):
            os.remove("./olricmeme.png")

        MemeYap(Resim, Text, font, Bottom, BottomText)
        await event.client.send_file(event.chat_id, "./olricmeme.png", reply_to=reply)
        await event.delete()
        os.remove(Resim)
    else:
...
CmdHelp('scrapers_bot').add_command(
    ...
    'meme', '<font> <üst;alt>', 'Fotoğrafa yazı ekleyin. İsterseniz font büyüklüğünü de yazabilirsiniz.', 'meme 30 olric;usta'
).add_command(
